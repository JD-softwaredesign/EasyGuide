...
const recordHandler = (e) => {
  chrome.storage.local.get('recordTargets', items => {
    ...
    if (items.recordTargets.length === 0) {
      elementData.push(window.location.href);
    }
    elementData.push({id: e.target.id,
...
const guideHandler = () => {
  chrome.storage.local.get(['currentGuide', 'currentStep'], items => {
    ...
    if (currentStep < currentGuide.length && matchedEl) {
      ...
      // needs to figure out why the stroke doesn't penetrate other elements
      // needs to somehow remove mousemove event handler on the document when matchedEl click happens
