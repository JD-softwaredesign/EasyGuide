...
const guideHandler = () => {
  chrome.storage.local.get(['currentGuide', 'currentStep'], items => {
    ...
    if (currentStep === 0) {
      ...
      return;
    } else if (currentStep === currentGuide.length) {
    ...
    let classMatched = classList.length ? Array.from(document.querySelectorAll(joinedClassList)) : [];

    if (document.getElementById(id)) {
      matchedEl = document.getElementById(id);
    } else if (classMatched.length) {
    ...
    if (currentStep < currentGuide.length && matchedEl) {
      matchedEl.classList.add('guide-highlight');
